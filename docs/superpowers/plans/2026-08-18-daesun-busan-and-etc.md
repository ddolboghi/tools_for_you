...
빈 줄의 추가·삭제, 항목 앞 공백 두 칸의 변화, `2. 전환 및 추가주문` 이후 어떤 줄의 변화도 나타나면 안 된다.

`=====SM=====` 뒤의 담당자 보고문에는 이 태스크에서 **딱 한 줄이 늘어난다.** `갈매기 : 0T - 0%` 다음에 `기타 : 0T - 0%`가 생긴다. 담당자 보고문의 무학 부분이 데이터 프레임 전부를 순회하고 있어서, Step 1에서 무학에 기타를 넣으면 여기까지 따라오기 때문이다. 이 줄은 Task 4가 걸러내므로 최종 상태에는 없다. 지금은 정상이니 고치려 하지 말 것. 담당자 보고문에 이 한 줄 말고 다른 변화(부산 줄 등)가 있으면 잘못된 것이다.

diff에 위에 적은 것 말고 예상 밖의 줄이 하나라도 있으면 커밋하지 말고 원인을 찾는다.
